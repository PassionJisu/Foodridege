...
로그인 후 화면은 역할마다 홈 인사·마이페이지 메뉴가 달라집니다. 하단 탭(홈 · 친구카세 · 환승반찬 · MealPick)은 **모든 역할이 동일**합니다. 권한이 없는 탭을 누르면 안내 팝업만 뜨고, 현재 화면은 그대로입니다.
...
Naver Map API로 가게 위치에 음식 이미지 핀을 올립니다. 홈과 같은 **All / Halal / Vegan / Veget / Chinese** 카테고리로 핀을 걸러 볼 수 있습니다. Android/iOS에서는 네이버맵 SDK, Windows 등에서는 SDK가 없어 정적 지도/핀으로 대체합니다.
