...
3. `driver@foodridge.kr` → 수거 동선·입고·수거 목록
4. `org@foodridge.kr` → 환승반찬 입고(수거) 신청
5. `owner@foodridge.kr` → MealPick에서 Add menu item
6. 회원가입에서 대학생 → 외국인(교환학생) → 체류 종료일을 어제 이전으로 두면, 다시 로그인할 때 자동 탈퇴 메시지를 확인할 수 있습니다
7. 회원가입에서 청년 → 외국인 청년은 ARC가 필수이고 학번은 없습니다. 대학생 외국인은 학번이 필수이고 ARC는 없습니다
